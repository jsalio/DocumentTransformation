...
{
    public sealed class DirectLock
    {
        private readonly IRequest<AddAttemptRequest> _request;
        private readonly IAttemptStore _attemptStore;

        public DirectLock(IAttemptStore attemptStore, IRequest<AddAttemptRequest> request)
        {
            _request = request;
        ...
        public Option<Task<int>,StoreException> Execute()
        {
            ...
            try
            {
                var query = _attemptStore.DirectLockCase(request, AttemptCaseStatus.Lock);
